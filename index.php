...
<div class="col-lg-2 col-2">
  ...
  $sql = "SELECT ID, miniaturka_okladki, tytul FROM ksiazka WHERE ocena >= 4 ORDER BY rand () limit 1 ";
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
      echo '<a href="selected.php?data='.$row['ID'].'"  class="profile">';
  ...
  $sql = "SELECT lp, zdjecie, imie, nazwisko, ocena FROM autor  WHERE ocena >= 4 ORDER BY rand () limit 1";
  $result = $conn->query($sql);
    if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
      echo '<a href="autor.php?data='.$row['lp'].'"  class="profile">';
      echo '<h2 class="profile__name">najpopularniejszy autor</h2>';
...
if ($kategorie) { 
$sql = "SELECT ksiazka.ID, ksiazka.tytul, ksiazka.miniaturka_okladki, ksiazka.krotki_opis, ksiazka.autor,autor.lp, autor.imie, autor.nazwisko FROM ksiazka  INNER JOIN autor ON ksiazka.autor = autor.lp WHERE Kategoria = $kategorie OR Podkategoria = $kategorie OR bestseller = $kategorie";
$result = $conn->query($sql);
$b = 0;
if ($result->num_rows > 0) {
  while($row = $result->fetch_assoc()) {
...
$sql = "SELECT ksiazka.ID, ksiazka.tytul, ksiazka.miniaturka_okladki, ksiazka.krotki_opis, ksiazka.autor,autor.lp, autor.imie, autor.nazwisko FROM ksiazka INNER JOIN autor ON ksiazka.autor = autor.lp WHERE ksiazka.tytul LIKE '%".$search."%' OR autor.imie LIKE '%".$search."%' OR autor.nazwisko LIKE '%".$search."%'";
$result= $conn -> query($sql);
if ($result->num_rows > 0) {
  while($row = $result->fetch_assoc()) {
...
// elseif(isset($_POST['sear'])){
// $sql = "SELECT lp, zdjecie, imie, nazwisko, ocena FROM autor";
// $result= $conn -> query($sql);
// if ($result->num_rows > 0) {
//   while($row = $result->fetch_assoc()) {

// echo '<div class="pozycja"><br> <a href="autor.php?data='.$row['lp'].'">'.$row["imie"]. ' ' .$row["nazwisko"]. '</a></p></div>';
// echo '<div class="row">';
// echo '<img class="col-12" src="data:zdjecie/jpeg;base64,'.base64_encode($row['zdjecie']).'"/>';
// echo '</div></form>';  
// }
// }
// }
...
if (isset($_GET['data'])){
  $i = $_GET['data'];
  $z = $i;
  $sql = "SELECT ksiazka.ID, ksiazka.tytul, ksiazka.miniaturka_okladki, ksiazka.krotki_opis, ksiazka.autor,autor.lp, autor.imie, autor.nazwisko FROM ksiazka INNER JOIN autor ON ksiazka.autor = autor.lp WHERE ID >= $i ORDER BY ID ASC";
  $result = $conn->query($sql);
}
  else{
  $sql = "SELECT ksiazka.ID, ksiazka.tytul, ksiazka.miniaturka_okladki, ksiazka.krotki_opis, ksiazka.autor,autor.lp, autor.imie, autor.nazwisko FROM ksiazka INNER JOIN autor ON ksiazka.autor = autor.lp ORDER BY ID ASC";
  $result = $conn->query($sql);
  $i = 1;
  }
  $d = $i + 9; 
  if ($result->num_rows > 0) {
    ...
    if (isset($_GET['data'])){
      if (($_GET['data']) > 1 ){
        $z = $z - 9;  
        echo '<div class="col"><center><a href=index.php?data='.$z.'>Poprzednia strona</a></center></div>';
      }}
    if ($i >= $d){
      echo '<div class="col"><center><a href=index.php?data='.$i.'>Nastepna strona</a></center></div>';
    }
    echo '</div>';    
    }}
